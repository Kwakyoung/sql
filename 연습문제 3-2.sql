...
-- 2. 두 날짜 사이의 개월수를 계산, 반환 : MONTHS_BETWEEN(date1, date2) / date1 : 이후날짜

-- 3. 원하는 컬럼 조회
-- 버림, 반올림 함수 ==> 소수점 첫째 자리 or 정수
SELECT  employee_id, first_name, salary, 
        ROUND(MONTHS_BETWEEN(SYSDATE, hire_date),0) AS "근무 개월 수1 반올림", 
        TRUNC(MONTHS_BETWEEN(SYSDATE, hire_date),0) AS "근무 개월 수2"
FROM    employees  -- 1. 테이블을 먼저 찾는다.
WHERE   department_id = 30;  -- 2. 필터링  - 6 rows
...
SELECT  employee_id, first_name||' '||last_name name, TO_CHAR(salary,'99,999') AS 급여
FROM    employees
WHERE   salary>=12000
...
-- 2004년 12월 31일 까지
-- 2005년 1월 1일 미만
-- NEXT_DAT(date, '요일형식') : 돌아오는 요일
-- LAST_DAY(date) : date의 달에 마지막 일자를 반환하는 함수

SELECT  employee_id, first_name||' '||last_name name, hire_date, TO_CHAR(hire_date, 'day')  AS "업무 시작 요일"
FROM    employees
