...
-- ASC : Ascending / 오름차순 : 작은값 --> 큰값
-- DESC : Descending / 내림차순 : 큰값 --> 작은값
-- ORDER BY는 너무 많이 사용하지 않는게 되려 성능 향상에 도움된다.
-- 데이터 베이스 내부에서 테이블 작성할때 기본적으로 테이블의 식별자를 기준으로 정렬되게 해주는데,
...
--ORDER BY first_name;  기본값이 ASC이기 때문에 생략가능
ORDER BY first_name ASC; -- 오름차순 


[예제2-40] 80번 부서의 사원정보에 대해, 이름을 내림차순으로 정렬한다.
SELECT employee_id, first_name, salary, department_id
FROM    employees
WHERE   department_id = 80
--ORDER BY first_name;  기본값이 ASC이기 때문에 생략가능
...
--=====================================================================
-- ORDER BY 절에 컬럼명 또는 컬럼의 Alias(=별칭, 약어)를 이용해 정렬가능
--       "       컬럼의 위치값을 이용해
--=====================================================================
