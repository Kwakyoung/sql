...
SELECT  *
FROM    employees
WHERE   commission_pct IS NOT NULL;  커미션율이 NULL 아닌사람 ==> 판매부서원만 커미션 받음!
*/
...
SELECT  TO_CHAR(ROUND(AVG(salary),2),'999,999') AS "90번 부서의 평균 급여액1",
        ROUND(AVG(salary),2) AS "90번 부서의 평균 급여액2"
FROM    employees
